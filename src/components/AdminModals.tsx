            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input
                  ...
                  placeholder={pIsProcurement ? "8자리 조달식별번호 입력" : "조달등록 제품 체크 시 활성화"}
                  className="w-full text-xs px-4 py-2.5 border border-neutral-200 rounded-xl focus:outline-none focus:border-neutral-950 font-mono disabled:bg-neutral-100 disabled:text-neutral-400 disabled:cursor-not-allowed transition-colors"
      ...
      <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1 text-left font-sans">
            {/* 3 Core Info Inputs (제품명, 현장명, 규격) */}
            <div className="space-y-4 bg-neutral-50 p-4 rounded-2xl border border-neutral-200/60">
              <span className="text-[11px] font-bold text-neutral-900 tracking-wider uppercase block">
                1. 기본 3대 정보 (제품명 / 현장명 / 규격)
              </span>

              <div>
                <label className="block text-xs font-bold text-neutral-600 mb-1">제품명 *</label>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="예: 디자인 등벤치 DADM-B10"
                  className="w-full text-xs px-3.5 py-2.5 border border-neutral-200 bg-white rounded-xl focus:outline-none focus:border-neutral-950 font-sans font-bold"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-neutral-600 mb-1">현장명 *</label>
                  <input
                    type="text"
                    value={location}
                    onChange={(e) => setLocation(e.target.value)}
                    placeholder="예: 용인 역북 푸르지오 신축 현장"
                    className="w-full text-xs px-3.5 py-2.5 border border-neutral-200 bg-white rounded-xl focus:outline-none focus:border-neutral-950"
                    required
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-neutral-600 mb-1">규격 *</label>
                  <input
                    type="text"
                    value={items}
                    onChange={(e) => setItems(e.target.value)}
                    placeholder="예: W2000 x D600 x H450 mm"
                    className="w-full text-xs px-3.5 py-2.5 border border-neutral-200 bg-white rounded-xl focus:outline-none focus:border-neutral-950"
                    required
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                <div>
                  <label className="block text-xs font-bold text-neutral-500 mb-1">분류 태그</label>
                  <input
                    type="text"
                    value={tag}
                    onChange={(e) => setTag(e.target.value)}
                    placeholder="예: 공동주택 조경"
                    className="w-full text-xs px-3 py-2 border border-neutral-200 bg-white rounded-xl focus:outline-none focus:border-neutral-950"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-neutral-500 mb-1">시공/공사 기간</label>
                  <input
                    type="text"
                    value={period}
                    onChange={(e) => setPeriod(e.target.value)}
                    placeholder="예: 2026.04 - 2026.05"
                    className="w-full text-xs px-3 py-2 border border-neutral-200 bg-white rounded-xl focus:outline-none focus:border-neutral-950 font-mono"
                  />
                </div>
              </div>
            </div>
              ...
              <div className="bg-neutral-50 p-4 rounded-2xl border border-neutral-200/60 space-y-2">
                ...
                <div className="flex gap-2.5 items-center">
                  {image && (
                    <div className="w-14 h-14 bg-neutral-100 rounded-lg overflow-hidden border border-neutral-200 shrink-0">
                      <img
                        ...
                        className="w-full h-full object-cover"
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = 'https://picsum.photos/seed/fallback-c1/200/200';
                        }}
                        referrerPolicy="no-referrer"
                      />
                    </div>
                  ...
                  <input
                    ...
                    onBlur={(e) => {
                      const converted = convertSynologyToDirectUrl(e.target.value);
                      if (converted !== e.target.value) setImage(converted);
                    }}
                    placeholder="사진 1 URL 주소 입력"
              ...
              <div className="bg-neutral-50 p-4 rounded-2xl border border-neutral-200/60 space-y-2">
                ...
                <div className="flex gap-2.5 items-center">
                  {image2 && (
                    <div className="w-14 h-14 bg-neutral-100 rounded-lg overflow-hidden border border-neutral-200 shrink-0">
                      <img
                        src={getDirectImageUrl(image2)}
                        alt="사진 2 미리보기"
                        className="w-full h-full object-cover"
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = 'https://picsum.photos/seed/fallback-c2/200/200';
                        }}
                        referrerPolicy="no-referrer"
                      />
                    </div>
                  ...
                  <input
                    ...
                    onBlur={(e) => {
                      const converted = convertSynologyToDirectUrl(e.target.value);
                      if (converted !== e.target.value) setImage2(converted);
                    }}
                    placeholder="사진 2 URL 주소 입력"
                    className="w-full text-xs px-3.5 py-2.5 border border-neutral-200 bg-white rounded-xl focus:outline-none focus:border-neutral-950 font-mono"
                  />
                  <label className="shrink-0 bg-neutral-900 hover:bg-neutral-800 text-white text-[11px] font-bold px-3 py-2.5 rounded-xl cursor-pointer transition-colors inline-flex items-center space-x-1 shadow-xs">
                    <Upload size={13} />
