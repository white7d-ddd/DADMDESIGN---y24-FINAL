...
export default function App() {
  ...
  const [activePage, setActivePage] = useState<'home' | 'products' | 'procurement' | 'about' | 'inquiry' | 'admin' | 'construction' | 'cases'>('home');
  ...
  const safeSetLocalStorage = (key: string, data: any) => {
    try {
      ...
      console.warn(`[Storage Warning] Could not cache "${key}" to localStorage:`, err);
    }
  };
  ...
  const handleUpdateAvailableIcons = (updatedIcons: { name: string; label: string }[]) => {
    ...
    safeSetLocalStorage('dadm_available_icons', updatedIcons);
    saveServerDB({ availableIcons: updatedIcons });
  };
  ...
  const handleUpdateHomeSectionInfoDirectly = (updatedInfo: HomeSectionInfo) => {
    ...
    safeSetLocalStorage('dadm_home_section', updatedInfo);
    saveServerDB({ homeSectionInfo: updatedInfo });
  };

  const handleUpdatePageHeaders = (updatedHeaders: PageHeaders) => {
    setPageHeaders(updatedHeaders);
    safeSetLocalStorage('dadm_page_headers', updatedHeaders);
    saveServerDB({ pageHeaders: updatedHeaders });
  };

  const handleUpdatePopups = (updatedPopups: PopupItem[]) => {
    setPopups(updatedPopups);
    safeSetLocalStorage('dadm_popups', updatedPopups);
  ...
  // 1. Initial hydration from local Storage & Server DB
  useEffect(() => {
    ...
    if (cachedProducts) {
      ...
      setProducts(defaultProducts);
      safeSetLocalStorage('dadm_products', defaultProducts);
    }
    ...
    if (cachedCategories) {
      ...
      setCategories(defaultCategories);
      safeSetLocalStorage('dadm_categories', defaultCategories);
    }
    ...
    if (cachedBanners) {
      ...
      setBanners(defaultBanners);
      safeSetLocalStorage('dadm_banners', defaultBanners);
    }
    ...
    if (cachedCompany) {
      ...
      setCompanyInfo(defaultCompanyInfo);
      safeSetLocalStorage('dadm_company', defaultCompanyInfo);
    }
    ...
    if (cachedHomeSection) {
      ...
      setHomeSectionInfo(defaultHomeSectionInfo);
      safeSetLocalStorage('dadm_home_section', defaultHomeSectionInfo);
    }
    ...
    if (cachedInquiries) {
      ...
      setInquiries(INITIAL_MOCK_INQUIRIES);
      safeSetLocalStorage('dadm_inquiries', INITIAL_MOCK_INQUIRIES);
    }
    ...
    if (cachedPopups) {
      ...
      setPopups(defaultPopups);
      safeSetLocalStorage('dadm_popups', defaultPopups);
    }
    ...
    if (cachedPageHeaders) {
      ...
      setPageHeaders(defaultPageHeaders);
      safeSetLocalStorage('dadm_page_headers', defaultPageHeaders);
    }
    ...
    // Fetch persistent server data from /api/db to synchronize
    fetch('/api/db')
      ...
      .then(db => {
        ...
        if (Array.isArray(db.banners) && db.banners.length > 0) {
          ...
          safeSetLocalStorage('dadm_banners', db.banners);
        }
        if (db.companyInfo && typeof db.companyInfo === 'object') {
          setCompanyInfo(db.companyInfo);
          safeSetLocalStorage('dadm_company', db.companyInfo);
        }
        if (db.homeSectionInfo && typeof db.homeSectionInfo === 'object') {
          setHomeSectionInfo(db.homeSectionInfo);
          safeSetLocalStorage('dadm_home_section', db.homeSectionInfo);
        }
        if (db.pageHeaders && typeof db.pageHeaders === 'object') {
          setPageHeaders(db.pageHeaders);
          safeSetLocalStorage('dadm_page_headers', db.pageHeaders);
        }
        if (Array.isArray(db.inquiries)) {
          setInquiries(db.inquiries);
          safeSetLocalStorage('dadm_inquiries', db.inquiries);
        }
        if (Array.isArray(db.constructionProjects) && db.constructionProjects.length > 0) {
        ...
        if (db.adminCredentials && db.adminCredentials.username && db.adminCredentials.password) {
          ...
          safeSetLocalStorage('dadm_admin_password', db.adminCredentials.password);
        }
      })
      .catch(err => console.warn('Could not load /api/db:', err));
  }, []);
  ...
  const handleUpdateBanners = (updatedBanners: Banner[]) => {
    ...
    safeSetLocalStorage('dadm_banners', updatedBanners);
    saveServerDB({ banners: updatedBanners });
  };

  const handleUpdateCompanyInfo = (updatedCompany: CompanyInfo) => {
    setCompanyInfo(updatedCompany);
    safeSetLocalStorage('dadm_company', updatedCompany);
    saveServerDB({ companyInfo: updatedCompany });
  };

  const handleUpdateInquiries = (updatedInquiries: Inquiry[]) => {
    setInquiries(updatedInquiries);
    safeSetLocalStorage('dadm_inquiries', updatedInquiries);
    saveServerDB({ inquiries: updatedInquiries });
  };

  const handleUpdateCategories = (updatedCategories: Category[]) => {
    setCategories(updatedCategories);
    safeSetLocalStorage('dadm_categories', updatedCategories);
    saveServerDB({ categories: updatedCategories });
  };
    ...
    <div className="min-h-screen bg-white flex flex-col font-sans text-neutral-800 antialiased selection:bg-neutral-900 selection:text-white">
      ...
      <main className="flex-grow">
        <AnimatePresence mode="wait">
          ...
          {/* CONSTRUCTION PORTFOLIO / CASES PAGE */}
          {(activePage === 'construction' || activePage === 'cases') && (
            <motion.div
              key={activePage}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
